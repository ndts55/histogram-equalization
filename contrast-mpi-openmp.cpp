...
#pragma omp parallel default(none) shared(partial_histogram, img_chunk_length, partial_img)
    {
#pragma omp for
        for (int &i: partial_histogram)i = 0;
#pragma omp for reduction(+:partial_histogram)
...
#pragma omp parallel for default(none) shared(img_chunk_length, partial_equalised, lookup, partial_img)
    for (auto i = 0; i < img_chunk_length; i++) {
...
#pragma omp parallel for default(none) shared(chunk_length, pr, pg, pb, ph, ps, pl)
...
int hsl_to_ppm(HSL_IMG &hsl, int rank, int world_size, PPM_IMG *ppm) {
    ...
    auto pg = new unsigned char[chunk_length];
    auto pb = new unsigned char[chunk_length];
#pragma omp parallel for default(none) shared(chunk_length, ph, ps, pl, pr, pg, pb)
    for (auto i = 0; i < chunk_length; i++) {
        auto h = ph[i];
        auto s = ps[i];
...
#pragma omp parallel for default(none) shared(chunk_length, pr, pg, pb, py, pu, pv)
    for (auto i = 0; i < chunk_length; i++) {
        auto r = pr[i];
        auto g = pg[i];
        auto b = pb[i];
...
int yuv_to_ppm(YUV_IMG &yuv, int rank, int world_size, PPM_IMG *ppm) {
    ...
    auto pg = new unsigned char[chunk_length];
    auto pb = new unsigned char[chunk_length];
#pragma omp parallel for default(none) shared(chunk_length, py, pu, pv, pr, pg, pb)
    for (auto i = 0; i < chunk_length; i++) {
        auto y = (int) py[i];
        auto cb = pu[i] - 128;
        auto cr = pv[i] - 128;
...
#pragma omp parallel for default(none) shared(result, ibuf)
...
void write_ppm(PPM_IMG img, const char *path) {
    ...
    auto *obuf = (unsigned char *) malloc(3 * img.w * img.h * sizeof(unsigned char));

#pragma omp parallel for default(none) shared(img, obuf)
